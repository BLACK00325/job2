...
-- دالة موجودة عندك للتحقق من المفتاح
local function CheckPlayerHasKey(citizenid, plate)
    -- هنا الدالة اللي كتشيك واش اللاعب عندو المفتاح
    -- رجع true أو false
end

-- Event لإعطاء المفتاح
RegisterNetEvent('MojiaVehicleKeys:server:givekey', function(plate)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    AddKeyToDatabase(Player.PlayerData.citizenid, plate)
end)

-- Callback موجود إذا بغيت تستخدمه
QBCore.Functions.CreateCallback('MojiaVehicleKeys:server:HasKey', function(source, cb, plate)
    local Player = QBCore.Functions.GetPlayer(source)
    if not Player then cb(false) return end
    cb(CheckPlayerHasKey(Player.PlayerData.citizenid, plate))
end)

-- **Export رسمي باش أي resource آخر يقدر يستعمله**
exports('HasKey', function(source, plate)
    local Player = QBCore.Functions.GetPlayer(source)
    if not Player then return false end
    return CheckPlayerHasKey(Player.PlayerData.citizenid, plate)
end)
